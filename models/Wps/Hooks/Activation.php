...
class Wps_Hooks_Activation
{
    ...
    public function install()
    {
        ...
        // Grab the Current version, if it's really old it might be in
        // 'csd_version'
        $this->_currentVersion = (get_option('wps_version') ? get_option('wps_version') : get_option('csd_version'));
        
        if ($this->_currentVersion != Wps_Plugin::VERSION) {
            ...
            update_option('wps_version', Wps_Plugin::VERSION);
        }
    ...
    protected function _upgrade ()
    {
        ...
        switch ($this->_currentVersion) {
            case '':
                ...
                update_option('csd_catarchives_on', 
                (get_option('sr_catarchives_on') ? get_option('sr_catarchives_on') : ''));
            ...
            case '0.5.0':
                ...
                $wpdb->query(
                "UPDATE " . $wpdb->postmeta . " SET meta_key = 'wps_page_theme' WHERE meta_key = 'csd_page_theme'");
                $wpdb->query(
                "UPDATE " . $wpdb->postmeta .
                 " SET meta_key = 'wps_page_subdomain' WHERE meta_key = 'csd_page_subdomain'");
                $wpdb->query(
                "UPDATE " . $wpdb->postmeta .
                 " SET meta_key = 'wps_tie_to_category' WHERE meta_key = 'csd_tie_to_category'");
            case '0.5.1':
        ...
        if (version_compare($this->_currentVersion, '1.0.1', '<')) {
            // Update meta values from 'true' to 1
            $wpdb->query(
            "UPDATE " . $wpdb->postmeta .
             " SET meta_value = '1' WHERE meta_key = 'wps_page_subdomain' and meta_value = 'true'");
            $wpdb->query(
            "UPDATE " . $wpdb->postmeta . " SET meta_value = '1' WHERE meta_key = 'wps_showall' and meta_value = 'true'");
            $wpdb->query(
            "UPDATE " . $wpdb->postmeta .
             " SET meta_value = '1' WHERE meta_key = 'wps_on_main_index' and meta_value = 'true'");
            
            // Update meta names so they are hidden
            $wpdb->query(
            "UPDATE " . $wpdb->postmeta . " SET meta_key = '_wps_page_theme' WHERE meta_key = 'wps_page_theme'");
            $wpdb->query(
            "UPDATE " . $wpdb->postmeta . " SET meta_key = '_wps_page_subdomain' WHERE meta_key = 'wps_page_subdomain'");
            $wpdb->query(
            "UPDATE " . $wpdb->postmeta . " SET meta_key = '_wps_tie_to_category' WHERE meta_key = 'wps_tie_to_category'");
            $wpdb->query("UPDATE " . $wpdb->postmeta . " SET meta_key = '_wps_showall' WHERE meta_key = 'wps_showall'");
            $wpdb->query(
            "UPDATE " . $wpdb->postmeta . " SET meta_key = '_wps_on_main_index' WHERE meta_key = 'wps_on_main_index'");
        }
    }
}