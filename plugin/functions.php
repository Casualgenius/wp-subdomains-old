<?php
/*
 * function wps_blogurl() { $url = get_option ( 'home' ); $url = substr ( $url,
 * 7 ); $url = str_replace ( "www.", "", $url ); return $url; }
 */
...
function wps_get_authors ($exclude_admin = false)
{
    ...
    "SELECT ID, user_nicename, display_name from $wpdb->users " . ($exclude_admin ? "WHERE user_login <> 'admin' " : '') .
...
/*
 * function sd_posts_where( $where ) { global $wp_query, $wpdb,
 * $csd_timeofchange, $cat; $append = $_SERVER[ "REQUEST_URI" ]; if ( strpos (
 * $append, "archives/" ) == 1 ) { $matches = array (); preg_match (
 * "/archives\/(.*)\/(.*)\/(.*)\.html/", $append, $matches ); $post =
 * $wpdb->get_row ( "SELECT * FROM $wpdb->posts WHERE post_name='{$matches[3]}'"
 * ); $postID = $post->ID; $post_date = $post->post_date; if ( intval ( $postID
 * ) && strtotime ( $post_date ) < $csd_timeofchange ) { $where = " AND
 * ID=$postID "; $wp_query->is_single = true; $wp_query->is_404 = false; unset (
 * $wp_query->query_vars ["error"] ); $wp_query->query = "name=" . $matches [3];
 * $wp_query->query_vars ["name"] = $matches [3]; } } return $where; }
 */

// Wordpress's Canonical Redirect doesn't really care much about subdomains so
// this not really needed
function wps_redirect_canonical ($redirect_url, $requested_url)
{
    /*
     * // Grab the hostname of the requested url $parsed_url =
     * parse_url($requested_url); $requested_host = $parsed_url['host'];
     * $subdomains = explode ( ".", $requested_host ); $subdomain =
     * $subdomains[0];
     */
	/*
	$blogurl = wps_blogurl ();
